          ...
          <div class="nav-scroll-wrapper">
            <ul class="secondary-nav-links">
              <li class="secondary-nav-item" v-for="(item, index) in secondaryNavItems" :key="index">
                <router-link :to="item.link" class="secondary-nav-link" @mouseenter="activateHover(index)">
        ...
        <div class="mobile-nav-section">
          ...
          <ul class="mobile-nav-links">
            <li class="mobile-nav-item">
              <router-link class="mobile-nav-link" to="/contact" @click="closeMobileMenu">
                <v-icon>mdi-phone-outline</v-icon>
                <span>Contact</span>
              </router-link>
            </li>
        ...
        <div class="mobile-nav-section">
          ...
          <ul class="mobile-secondary-links">
            <li class="mobile-secondary-item" v-for="(item, index) in secondaryNavItems" :key="index">
              <router-link :to="item.link" class="mobile-secondary-link" @click="closeMobileMenu">
                <v-icon>{{ item.icon }}</v-icon>
                <span>{{ item.text }}</span>
              </router-link>
            </li>
...
const dialogInstructions = ref(false)
...
const sendInquiry = async () => {
  ...
  const selectedBranchConfig = branchConfig[formData.value.branch]
  ...
  const templateParams = {
    ...
    contact: formData.value.contact || "Not provided",
    branch: formData.value.branch,
    source: formData.value.source || "Not provided",
...
const proceedToPayment = () => {
  dialogInstructions.value = false
...
.mobile-contact-item :deep(.v-icon) {
  color: #FFD700;
}
...
/* Ensure proper v-icon styling */
:deep(.v-icon) {
  font-size: 1.25rem;
}

/* Fix for v-btn text prop in Vue 3 */
:deep(.v-btn) {
  text-transform: none;
  font-weight: 600;
  font-family: 'Poppins', sans-serif;
